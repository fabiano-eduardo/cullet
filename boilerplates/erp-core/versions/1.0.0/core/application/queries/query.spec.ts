...
class FindEntityQuery extends Query<{ id: string }, string | null> {
	protected execute(input: { id: string }): string | null {
		// in production this would query the repository
		return input.id === 'known' ? 'entity-data' : null;
	}
}

class CountQuery extends Query<void, number> {
...
describe('Query', () => {
	...
	it('executes via run() and returns the output of execute()', async () => {
		...
		expect(await query.run({ id: 'known' })).toBe('entity-data');
		expect(await query.run({ id: 'unknown' })).toBeNull();
	});
