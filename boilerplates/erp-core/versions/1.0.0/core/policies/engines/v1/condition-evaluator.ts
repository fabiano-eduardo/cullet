...
export class ConditionEvaluatorV1 {
	...
	private evaluateWithTraceInternal(
		...
		if (ConditionEvaluatorV1.isAndNode(node)) {
			...
			const lastIndex = node.and.length - 1;
			const lastChild = node.and[lastIndex]!;
			const lastChildResult = this.evaluateWithTraceInternal(
				lastChild,
				`${path}.and[${lastIndex}]`
			);
			if (lastChildResult.isErr()) {
				return Result.err(lastChildResult.errorOrNull()!);
			}

			return Result.ok({
				matched: true,
				trace: lastChildResult.getOrNull()!.trace,
			});
		}
