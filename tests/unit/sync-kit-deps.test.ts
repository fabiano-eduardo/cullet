...
async function importSyncKitDepsEntrypoint() {
  const scriptUrl = new URL("../../scripts/sync-kit-deps.mjs", import.meta.url);
...
describe("syncKitPeerDependencies", () => {
  it("reports drift in check mode without rewriting manifests", async () => {
    ...
    const result = await syncKitPeerDependencies(packagesRoot, { check: true });
    const manifest = JSON.parse(
      await readFile(join(packagesRoot, "erp-core", "package.json"), "utf8"),
    ) as {
  ...
  it("rewrites manifests from meta.json and removes empty peerDependencies", async () => {
    ...
    const dummyApiManifest = JSON.parse(
      await readFile(join(packagesRoot, "dummy-api", "package.json"), "utf8"),
    ) as {
      peerDependencies?: Record<string, string>;
    };
