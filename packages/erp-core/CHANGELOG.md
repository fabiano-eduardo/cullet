...
- 4ac6b57: Align the policy catalog with the kit's own error model: it now throws a domain exception instead of an application error.
...
- 0036266: Harden RBAC/ABAC, the policy condition engine, bitemporal primitives, the Result/Outcome module and the domain-exception hierarchy, plus fix several invariant bugs found while doing so.
...
- e9c453b: Add a zod-free ABAC module and the `./abac` subpath.
...
- e9c453b: Add a zod-free RBAC module and the `./rbac` subpath.
