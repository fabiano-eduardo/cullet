...
describe("temporal-range", () => {
  it('isOpen identifies ranges without "to"', () => {
    expect(
      isOpen(createValidTime({ from: new Date("2026-01-01T00:00:00.000Z") })),
    ).toBe(true);
  });
  ...
  it("contains includes the start date", () => {
    ...
    expect(contains(range, new Date("2026-01-01T00:00:00.000Z"))).toBe(true);
  });
  ...
  it("contains treats the end bound as exclusive", () => {
    ...
    expect(contains(range, new Date("2026-02-01T00:00:00.000Z"))).toBe(false);
  });
  ...
  it("contains works for open ranges", () => {
    ...
    expect(contains(range, new Date("2027-01-01T00:00:00.000Z"))).toBe(true);
  });
