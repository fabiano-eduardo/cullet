...
describe("UuidIdentifier", () => {
    describe("isValid()", () => {
        it("accepts a canonical v1–v5 UUID, case-insensitively", () => {
            expect(UuidIdentifier.isValid(VALID_UUID)).toBe(true);
            expect(UuidIdentifier.isValid(VALID_UUID.toUpperCase())).toBe(true);
        });

        it("rejects malformed strings, wrong versions and the nil UUID", () => {
            ...
            // version nibble 0 is outside the [1-5] range
            expect(
    ...
    describe("equality", () => {
        it("two ids holding the same UUID are equal", () => {
            expect(OrderId.create(VALID_UUID).equals(OrderId.create(VALID_UUID))).toBe(
                true,
            );
        });
        ...
        it("ids holding different UUIDs are not equal", () => {
            ...
            expect(OrderId.create(VALID_UUID).equals(OrderId.create(other))).toBe(
                false,
            );
        });
    });
