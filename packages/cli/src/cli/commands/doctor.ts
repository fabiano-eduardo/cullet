...
async function detectTypescriptVersion(projectRoot: string): Promise<{
    ...
    const installedPkg = await readJsonFile<{ version?: string }>(
        installedPath,
    );
                ...
                async handler(tracker) {
                    ...
                    if (tsconfigUnreadable) {
                        ...
                        const compilerOptions = readCompilerOptions(
                            tsconfigInspection.config as Record<
                                string,
                                unknown
                            >,
                        );
                        const moduleResolution =
                            readModuleResolution(compilerOptions);
                        ...
                        if (moduleResolution === null) {
                            ...
                            !ACCEPTED_MODULE_RESOLUTIONS.has(
                                moduleResolution.normalized,
                            )
                        ) {
                            findings.push({
...
function report(findings: Finding[]): void {
    ...
    for (const finding of findings) {
        ...
        console.log("");
        console.log(
            `${tag} ${pc.bold(`[${finding.code}]`)} ${finding.message}`,
        );
        if (finding.hint) {
