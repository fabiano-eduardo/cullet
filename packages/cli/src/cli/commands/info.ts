...
async function printPublishedVersions(npmName: string): Promise<void> {
    ...
    if (sorted.length > preview.length) {
        console.log(
            pc.dim(
                `  ... e mais ${sorted.length - preview.length} versao(oes)`,
            ),
        );
    }
                ...
                async handler(tracker) {
                    ...
                    if (tooling) {
                        if (importableTooling) {
                            console.log(
                                pc.bold("Importe direto (sem copiar):"),
                            );
                            console.log(
                                pc.cyan(
                        ...
                        console.log(
                            pc.cyan(`npx cullet fc ${name}@${version}`),
                        );
                        const placement = getCopyPlacement(meta);
                        if (placement !== undefined) {
                            console.log(
                                pc.dim(
                                    `Os arquivos sao copiados para: ${placement}`,
                                ),
                            );
                        }
                    } else {
                    ...
                    if (tooling) {
                        ...
                        if (importDeps.length > 0) {
                            console.log("");
                            console.log(
                                pc.bold("Peer deps para import direto:"),
                            );
                            for (const dependency of importDeps) {
                                console.log(
                                    pc.dim(`  ${formatDependency(dependency)}`),
                                );
                            }
                        }
                        ...
                        if (deps.length > 0) {
                            console.log("");
                            console.log(
                                pc.bold("Dependencias externas do payload:"),
                            );
