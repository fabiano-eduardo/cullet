...
export function matchKitArg(
    ...
    if (value.startsWith("@") && !value.includes("/")) {
        return BARE_LEADING_AT_NAME_PATTERN.test(value)
            ? { name: value }
            : null;
    }

    const match =
        SCOPED_KIT_ARG_PATTERN.exec(value) ??
        UNSCOPED_KIT_ARG_PATTERN.exec(value);
...
function parseKitCompatibility(value: unknown): KitCompatibility | undefined {
    ...
    if (
        !isRecord(engines) ||
        !isRecord(directImport) ||
        !isRecord(fullControl)
    ) {
        return undefined;
    }
