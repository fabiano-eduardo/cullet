...
export function kitSrcDir(packageRoot: string, name: string, version: string): string {
  return join(packageRoot, KITS_DIR, name, "versions", version);
}

export function kitDistDir(packageRoot: string, name: string, version: string): string {
  return join(packageRoot, DIST_DIR, "kits", name, "versions", version);
}

export function kitDistEntryRelative(name: string, version: string, ext: "js" | "d.ts"): string {
...
export function kitFullControlDir(projectCwd: string, name: string, version: string): string {
  return join(projectCwd, "cullet", `${name}@${version}`);
}

export function kitFullControlAliasTarget(name: string, version: string): string {
  return `./cullet/${name}@${version}/index.ts`;
}

export function kitImportSpecifier(name: string, version: string, isLatest: boolean): string {
