...
export function createTelemetryCommand(): Command {
  ...
  command
    ...
    .option(
      "--endpoint <url>",
      "Endpoint HTTP para exportar eventos alem do log local",
    )
    .action(async (options: EnableTelemetryOptions) => {
      const status = await enableTelemetry({ endpoint: options.endpoint });
      printStatus(status);
      console.log("");
      console.log(
        pc.dim(
          "Os eventos continuam sendo gravados localmente; o endpoint remoto e opcional e usa POST JSON por evento.",
        ),
      );
    });
