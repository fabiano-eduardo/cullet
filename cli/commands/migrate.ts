...
function printMigrationPlan(
  ...
  if (plan.guide !== undefined) {
    if (plan.guide.resolvedPath === null) {
      ...
      console.log(pc.cyan(`Guia de migracao: ${plan.guide.resolvedPath}`));
    }
  }
...
export function createMigrateCommand(): Command {
  return new Command("migrate")
    ...
    .option("--apply", "Executa o codemod declarado pelo kit")
    .option(
      "--dry-run",
      "Executa o codemod em modo simulacao (sem gravar mudancas)",
    )
