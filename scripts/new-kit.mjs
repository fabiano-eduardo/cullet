...
function parseArgs(argv) {
    ...
    if (!name)
        fail(
            "kit name is required. Usage: node scripts/new-kit.mjs <kit-name>",
        );
    ...
    if (description.length < 10) {
        fail(
            "description must be at least 10 characters (schema requirement).",
        );
    }
...
async function main() {
    ...
    if (registry[name] !== undefined) {
        fail(
            `kit "${name}" already exists in packages/cli/registry/index.json.`,
        );
    }
    ...
    console.log(
        `  4. run ${pc.cyan("npm run validate-kits")} to lint your kit`,
    );
    console.log(
        `  5. run ${pc.cyan(
